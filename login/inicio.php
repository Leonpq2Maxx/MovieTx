...
// forzar salida temprana
if (ob_get_level()) ob_flush();
flush();

/* ===== LÓGICA PESADA ===== */
// consultas MySQL, cálculos, etc
sleep(2);
/* ========================= */
...
<style>
    ...
    .alignleft {
        ...
        color: rgb(16, 235, 255, 0.461);
        cursor: pointer;
    }
    .alignleft:hover {
        background-color: rgb(16, 235, 255, 0.461);
        color: white;
    }
...
:root{
    ...
    /*card poster*/
    .alignleft{float:left;margin-left:5px; color:#000; font-weight:bold; font-size:16px;} 
    .alignright{float:right;margin-right:10px; color:#000; font-size:14px; font-weight:bold ;} 
    ...
    .xplus{position:relative;text-align:center;color:white;}
    /*.centered{ font-size:12px; font-weight:bold; position:absolute; top:80%; left: 50%; transform:translate(-50%,-50%)} 
    .cent{ font-size:auto; position:absolute ; top:95%; left: 50%; transform:translate(-50%,-50%)} */
    .xaviec{width:113px;border-radius:7px;height:150px;z-index:1;}
...
<!--TAMAÑO DE TEXTO Y CAMBIO DE COLOR-->
<style>
  .xplus {
    color: var(--headC);
  ...
  /* Disminuye todas las etiquetas <i> al 85% de su tamaño actual */
i {
  display: block;              /* Se comporta como un bloque para adaptarse al ancho */
  font-size: 0.68em;           /* Mantiene un tamaño cómodo para iconos/textos */
  line-height: 1.4;            /* Espacio cómodo entre líneas para legibilidad */
  white-space: normal;         /* Permite que el texto pase a otra línea */
  word-break: break-word;      /* Corta las palabras largas para adaptarse al contenedor */
  overflow-wrap: break-word;   /* Asegura que no desborde para texto extensos */
}
...
/* ======= CSS NAMESPACE: us_carousel (no conflict) ======= */
.us_carousel_wrap { width:100%; overflow:hidden; position:relative; margin: 18px 0; }
.us_carousel { position:relative; width:100%; overflow:hidden; }
.us_track { display:flex; transition: transform 0.6s ease; will-change: transform; }
.us_slide { width:100%; flex-shrink:0; object-fit:cover; height:55vh; }

/* Dots */
.us_dots { position:absolute; left:50%; bottom:14px; transform:translateX(-50%); display:flex; gap:8px; z-index:50; }
.us_dots button { width:10px; height:10px; border-radius:50%; border: none; background: rgba(255,255,255,0.45); padding:0; cursor:pointer; transition: transform .2s, background .2s; }
.us_dots button[aria-pressed="true"] { background: #f7b500; transform: scale(1.15); }

/* No-transition helper */
.us_no_transition { transition: none !important; }

/* Responsive adjustments */
@media (max-width:1024px) { .us_slide { height:50vh; } }
@media (max-width:768px) { .us_slide { height:36vh; } .us_dots { bottom:12px; } }
@media (max-width:480px) { .us_slide { height:27vh; } .us_dots { gap:6px; } }
...
<body class="oneGrd MN-2 mobS hdMn bD onIndx onHm" id="mainCont">
   ...
   <!-- Pantalla de carga MovieTx mejorada -->
<div id="loader-container">
  <div class="loader-content">
    ...
    <p class="porcentaje" id="porcentaje">0%</p>

    <p class="texto-carga">Cargando Peliculas y Series...</p>

    <p class="footer-loader">MovieTx © 2025 — Todos los derechos reservados</p>
...
#loader-container {
  position: fixed;
  inset: 0;
  background: linear-gradient(160deg, #000 0%, #001a2b 60%, #003d5c 100%);
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center; /* ESTO CENTRA TODO */
  color: white;
...
.loader-content {
  text-align: center;
  opacity: 0;
  animation: fadeOpacity 0.7s ease forwards; /* SOLO OPACIDAD */
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function actualizarLoader(valor) {
    ...
    function finalizarLoader() {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                // Activar carrusel si existe
                if (window.miCarruselSwiper) {
                    miCarruselSwiper.autoplay.start();
                }
    ...
    // ==========================================
    // 1 — Carga de lo esencial (logo)
    // ==========================================
    const logo = document.querySelector(".logo-movietx");

    if (logo.complete) {
        actualizarLoader(40);
    } else {
        logo.onload = () => actualizarLoader(40);
        logo.onerror = () => actualizarLoader(40);
    }

    // ==========================================
    // 2 — Avance "inteligente" del loader
    // ==========================================
    let anim = setInterval(() => {
        if (progreso < 90) {
            actualizarLoader(progreso + 5);
        }
    }, 120);

    // ==========================================
    // 3 — Tiempo máximo (Turbo Loader)
    // ==========================================
    setTimeout(() => {
        clearInterval(anim);
        finalizarLoader();
...
<li class="isDrk">
    <li class="isRefresh">
  <label aria-label="Refresh" class="tRefresh tIc bIc" id="refreshBtn">
    <svg class="line" viewBox="0 0 24 24">
      <path d="M21 12a9 9 0 1 1-3-6.7V3h2v6h-6V7h3.6A7 7 0 1 0 12 19a7 7 0 0 0 7-7h2z"></path>
    </svg>
  </label>
</li>
...
.top-scroll-menu {
  ...
  gap: 10px;
  padding: 10px 15px;
  background-color: var(--top-scroll-bg, #ffffff);
...
/* Enlaces */
.menu-item {
  color: var(--top-scroll-color, #000);
...
.menu-item.active {
    background-color: #00cfff !important; /* Celeste */
    color: white !important;
    border: 1px solid red !important; /* Mantiene borde rubio */
}
...
// CLICK → marcar la sección actual
botonesMenu.forEach(boton => {
  boton.addEventListener("click", () => {
    activarMenu(boton.id);
  });
});

// REFRESCAR PÁGINA → volver a Inicio SIEMPRE
window.addEventListener("DOMContentLoaded", () => {
  activarMenu("inicio");

  // Si usás carga dinámica de contenido:
...
<div class="us_carousel_wrap" id="us_carousel_wrap" aria-label="Carrusel de imágenes">
  <div class="us_carousel" id="us_carousel">
    <div class="us_track" id="us_track">
      <img class="us_slide" loading="lazy" src="https://image.tmdb.org/t/p/w780/t3GSbE7Zw8k5iIVFLpOPU206b9P.jpg" alt="Slide 1">
      <img class="us_slide" loading="lazy" src="https://image.tmdb.org/t/p/w780/fAV8StFLcssURq6EYwoMAP5tKwF.jpg" alt="Slide 2">
      <img class="us_slide" loading="lazy" src="https://image.tmdb.org/t/p/w780/v7ocuLE59xrKCnz21mCrwSo2xN4.jpg" alt="Slide 3">
      <img class="us_slide" loading="lazy" src="https://image.tmdb.org/t/p/w780/ybyObMOWerS5DgzaFbslvGi1ND3.jpg" alt="Slide 4">
      <img class="us_slide" loading="lazy" src="https://image.tmdb.org/t/p/w780/8WhlNrtQdyUBnKrw5WBJnsDn1HE.jpg" alt="Slide 5">
      <img class="us_slide" loading="lazy" src="https://image.tmdb.org/t/p/w780/6LPGdnB3iJ8VjpuCOfICkimFhTt.jpg" alt="Slide 6">
    </div>
...
(function(){
  ...
  const mappingAltToUrl = {
    "Slide 1": "View Peliculas/Until Dawn Noche de terror (2025).html",
    "Slide 2": "View Peliculas/Zootopia 2 (2025).html",
    "Slide 3": "View Series/Demon Slayer Kimetsu no Yaiba Infinity Castle (2025).html",
    "Slide 4": "View Peliculas/IT Bienvenidos a Derry (2025).html",
    "Slide 5": "View Peliculas/3 Culpa Nuestra (2025).html",
    "Slide 6": "View Peliculas/Baki Hanma vs. Kengan Ashura (2024).html"
...
<div id="contenido-orig">
  ...
  <div class="poster">
    <h4 class="alignleft">Hoy-Terror</h4><a href="#"></a>
  </div>

  <div class="scrollmenu">
    <br>
    <a class="card-link" data-href="View Peliculas/Until Dawn Noche de terror (2025).html">
      <div class="xplus">
        <img alt="" class="xaviec" loading="lazy" src="https://image.tmdb.org/t/p/w300/vAYTXSUnQjmTFcm97BhROQav1wF.jpg"/>
        <i>Until Dawn: Noche de terror</i>
    ...
    <a class="card-link" data-href="View Peliculas/Winnie the Pooh Sangre y miel 1 (2023).html">
      <div class="xplus">
        ...
        <i>Winnie the Pooh: Sangre y miel</i>
        <span class="lock-icon">🔒</span>
      </div>
    </a>
  ...
  <div class="poster">
    <h4 class="alignleft">Pronto</h4><a href="#"></a>
  </div>
  ...
  <div class="poster">
    <h4 class="alignleft">Serie</h4><a href="#"></a>
  </div>
  ...
  <div class="scrollmenu">
    ...
    <a class="card-link" data-href="View Series/Moises y los diez mandamientos (2015).html">
      <div class="xplus">
  ...
  <div class="poster">
    <h4 class="alignleft">Nuevas</h4><a href="#"></a>
  </div>
  ...
  <div class="scrollmenu">
    ...
    <a class="card-link" data-href="">
      <div class="xplus">
        ...
        <i>El Conjuro 4: El ultimo rito</i>
        <span class="lock-icon">🔒</span>
      </div>
    </a>
    
    <a class="card-link" data-href="View Peliculas/3 Culpa Nuestra (2025).html">
      <div class="xplus">
        <img class="xaviec" loading="lazy" src="https://image.tmdb.org/t/p/w300/6kmi6vmp6iOn4KzI7WfnVtAeJhU.jpg"/>
        <i>Culpa nuestra</i>
        <span class="lock-icon">🔒</span>
      </div>
    </a>
    
    <a class="card-link" data-href="View Peliculas/Como entrenar a tu dragon (2025).html">
      <div class="xplus">
...
const data = {
  "peliculas": `
    <div class="poster">
      <h4 class="alignleft">Pronto</h4><a href="#"><h4 class="alignright">Ver Todo</h4></a>
    </div>
    ...
    <div class="scrollmenu">
      ...
      <a class="card-link" data-href="View Peliculas/Kung Fu panda 4 (2024).html">
        <div class="xplus">
      ...
      <a class="card-link" data-href="View Peliculas/Intensamente 2.html">
        <div class="xplus">
    ...
    <div class="scrollmenu">
      ...
      <a class="card-link" data-href="View Peliculas/Tierra de osos (2003).html">
        <div class="xplus">
          <img alt="" class="xaviec" loading="lazy" src="https://image.tmdb.org/t/p/w300/xoEY7339ewJ4jvDZZqM3FKVJb8r.jpg"/>
          <i>Tierra de osos</i>
          <span class="lock-icon">🔒</span>
        </div>
      </a>
      ...
      <a class="card-link" data-href="View Peliculas/Tierra de osos 2 (2006).html">
        <div class="xplus">
    ...
    <div class="scrollmenu">
      ...
      <a class="card-link" data-href="View Peliculas/Deadpool y Wolverine (2024).html">
        <div class="xplus">
    ...
    <div class="scrollmenu">
      ...
      <a class="card-link" data-href="View Peliculas/Cantar desnuda (2025).html" data-adulto="adulto" onclick="handleAdultLinkClick(event)">
         <div class="xplus">
    ...
    <div class="scrollmenu">
      ...
      <a class="card-link" data-href="View Peliculas/Desafiante Rivales (2024).html">
        <div class="xplus">
  ...
  "series": `
    ...
    <div class="scrollmenu">
      ...
      <a class="card-link" data-href="">
        <div class="xplus">
          <img class="xaviec" loading="lazy" src="https://image.tmdb.org/t/p/w300/vC6LSYC8uhZPkPM01L6HKrr1lMD.jpg"/>
  ...
  "trailers": `
    ...
    <div class="scrollmenu">
      ...
      <a class="card-link" data-href="">
        <div class="xplus">
          <img alt="" class="xaviec" loading="lazy" src="https://image.tmdb.org/t/p/w300/38eo8njAYnMejGKoYHwvHxibHJZ.jpg"/>
          <i>Trailer: El conjuro 4</i>
          <span class="lock-icon">🔒</span>
        </div>
      </a> 
...
<nav class="menu-inferior">
  ...
  <a href="View Peliculas/favoritos.html"><i class="fas fa-heart"></i><span>Favoritos</span></a>
  <a href="Mostras Mas/Busqueda.html"><i class="fas fa-search"></i><span>Buscar</span></a>
  <a href="Menus Peliculas Series/Compartir.html"><i class="fas fa-share-alt"></i><span>Compartir</span></a>
  <a href="View Peliculas/historial_usuario.html"><i class="fas fa-history"></i><span>Historial</span></a>
</nav>
